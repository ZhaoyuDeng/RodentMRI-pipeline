function RatfMRI_alff_falff_ZZD(nii_file,brainmask,TR,Band,save_dir,Header)
...
alff_str = 'ALFFMap.nii';
alff_nii = fullfile(save_dir,alff_str);
falff_nii = fullfile(save_dir,['f',alff_str]);
out_nii = {alff_nii;falff_nii};
[ALFFBrain, fALFFBrain, Header] = y_alff_falff(nii_file,TR,Band(2),Band(1),brainmask,out_nii,Header);
BrainMaskData=double(ALFFBrain~=0);

%%%=====Post processing=====%%%
%%%=====ALFF=====%%%
Temp = ((ALFFBrain - mean(ALFFBrain(find(BrainMaskData)))) ./ std(ALFFBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
fname= fullfile(save_dir,['z',alff_str]);
y_Write(Temp,Header,fname);

Temp = (ALFFBrain ./ mean(ALFFBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
fname= fullfile(save_dir,['m',alff_str]);
y_Write(Temp,Header,fname);
Temp=Temp - 1;
fname= fullfile(save_dir,['m-1',alff_str]);
y_Write(Temp,Header,fname);
 
%%%=====fALFF=====%%%
Temp = ((fALFFBrain - mean(fALFFBrain(find(BrainMaskData)))) ./ std(fALFFBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
fname= fullfile(save_dir,['zf',alff_str]);
y_Write(Temp,Header,fname);

Temp = (fALFFBrain ./ mean(fALFFBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
fname= fullfile(save_dir,['mf',alff_str]);
y_Write(Temp,Header,fname);
Temp=Temp - 1;
fname= fullfile(save_dir,['m-1f',alff_str]);
y_Write(Temp,Header,fname);

function [ALFFBrain, fALFFBrain, Header] = y_alff_falff(AllVolume,ASamplePeriod, HighCutoff, LowCutoff, AMaskFilename, AResultFilename, Header, CUTNUMBER)
