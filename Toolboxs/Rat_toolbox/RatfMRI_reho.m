function RatfMRI_reho(nii_file,brainmask,save_dir,sm_kernel,Header)
...
if ~exist('sm_kernel','var')
    sm_kernel = [6 6 6];%smooth reho kernel
end
reho_str  = 'ReHoMap.nii';
reho_cluster = 27;
reho_nii = fullfile(save_dir,reho_str);

[ReHoBrain, Header] = y_reho(nii_file,reho_cluster,brainmask,reho_nii,Header);
BrainMaskData=double(ReHoBrain~=0);
Temp = (ReHoBrain ./ mean(ReHoBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
y_Write(Temp,Header,fullfile(save_dir,['m',reho_str]));
Temp = ((ReHoBrain - mean(ReHoBrain(find(BrainMaskData)))) ./ std(ReHoBrain(find(BrainMaskData)))) .* (BrainMaskData~=0);
y_Write(Temp,Header,fullfile(save_dir,['z',reho_str]));
 
flag=1;
if flag
    clear matlabbatch
    matlabbatch{1}.spm.spatial.smooth.data = {fullfile(save_dir,[reho_str]);
        fullfile(save_dir,['m',reho_str]);
        fullfile(save_dir,['z',reho_str])};    
    matlabbatch{1}.spm.spatial.smooth.fwhm = sm_kernel;
...
function [ReHoBrain, Header] = y_reho(AllVolume, NVoxel, AMaskFilename, AResultFilename, Header, CUTNUMBER)
